# Ivan Ajala - Portfólio Profissional em Ciência / Analise de Dados

[![Portfólio Data Science](https://img.shields.io/badge/Portfólio-Data_Science-2E86C1)](https://github.com/IvanAjala/ivan-ajala-ds-portfolio)
[![GitHub](https://img.shields.io/badge/GitHub-Projetos_Completos-181717)](https://github.com/IvanAjala)
[![Python](https://img.shields.io/badge/Python-3.9+-3776AB?logo=python&logoColor=white)](https://www.python.org/)
[![Machine Learning](https://img.shields.io/badge/Machine_Learning-Production_Ready-FF6F00)](https://scikit-learn.org/)
[![Status](https://img.shields.io/badge/Status-Open_to_Work-27AE60)](https://linkedin.com/in/ivan-alala-ba759223)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)

## 👨‍💻 Sobre Mim

**Cientista de Dados em transição estratégica** com mais de **20 anos de experiência em gestão de operações e estratégia empresarial**. Combino visão de negócios com habilidades técnicas avançadas para transformar dados em insights acionáveis e impacto mensurável.

**🎯 Objetivo Profissional:** Posições como **Cientista de Dados | Analista de Dados** onde possa aplicar minha dupla expertise (técnica + negócios) para resolver desafios complexos e gerar valor através de dados.

**📈 Meu Diferencial:** Entendo tanto a linguagem dos dados quanto a dos negócios. Traduzo problemas corporativos em soluções técnicas e comunico insights complexos para decisores executivos.

---

## 🚀 Projetos em Destaque

### [📊 01 - Sistema de Prevenção de Churn com ML Interpretável](projects/01-churn-prediction/)
**Problema de Negócio:** Reduzir a evasão de clientes através da identificação proativa de riscos  
**Objetivo:** Desenvolver modelo preditivo com interpretabilidade para ações de retenção personalizadas  
**Stack:** Python, XGBoost, SHAP, Streamlit, Azure ML, Power BI  
**Impacto:** Redução estimada de **25% na taxa de churn** | ROI 3.5x  
**Status:** ✅ **Completo** (Modelo + Dashboard + Deploy)  

**Principais Entregas:**
- Modelo Random Forest Calibrado com 84% ROC-AUC
- Dashboard interativo com análise individual e em lote
- Análise SHAP para explicabilidade do modelo
- Insights acionáveis para times de Marketing e Customer Success

### [🌍 02 - Análise de Sustentabilidade & ESG com Dados Públicos](projects/02-esg-analysis/)
**Problema de Negócio:** Avaliar desempenho ESG e correlacionar com indicadores econômicos  
**Objetivo:** Criar dashboard narrativo para análise de impacto ambiental e social  
**Stack:** Python, GeoPandas, Tableau, Plotly, SQL  
**Impacto:** Identificação de oportunidades em **mercados sustentáveis**  
**Status:** 🔄 **Em Desenvolvimento** (Fase de ETL)  

**Principais Entregas:**
- Integração de múltiplas fontes de dados públicos
- Análise espacial com mapas interativos
- Dashboard executivo com storytelling de dados
- Correlações entre métricas ESG e performance econômica

### [🛒 03 - Sistema de Recomendação para E-commerce](projects/03-recommendation-system/)
**Problema de Negócio:** Aumentar conversão e ticket médio através de personalização  
**Objetivo:** Desenvolver sistema de recomendação híbrido (colaborativo + conteúdo)  
**Stack:** Python, Surprise, NLP, FastAPI, Docker  
**Impacto:** Aumento estimado de **15-20% em conversão**  
**Status:** 🎯 **Planejamento Finalizado**  

**Principais Entregas:**
- Filtragem colaborativa e baseada em conteúdo
- Análise de sentimento em reviews (NLP)
- API REST para integração com sistemas existentes
- Métricas de negócio além de acurácia técnica

---

## 🛠️ Habilidades Técnicas

### **🧪 Ciência de Dados & Machine Learning**
![Python](https://img.shields.io/badge/Python-Expert-3776AB?logo=python&logoColor=white)
![Machine Learning](https://img.shields.io/badge/ML-Scikit_learn,_XGBoost-FF6F00)
![Deep Learning](https://img.shields.io/badge/Deep_Learning-TensorFlow-FF6F00)
![Estatística](https://img.shields.io/badge/Estatística-Avançada-8E44AD)

**Especializações:** Modelos Preditivos, Séries Temporais, NLP, Visão Computacional, Feature Engineering, MLOps

### **📊 Análise & Visualização de Dados**
![SQL](https://img.shields.io/badge/SQL-Avançado-4479A1?logo=postgresql&logoColor=white)
![Power BI](https://img.shields.io/badge/Power_BI-Expert-F2C811?logo=powerbi&logoColor=black)
![Tableau](https://img.shields.io/badge/Tableau-Intermediário-E97627?logo=tableau&logoColor=white)

**Ferramentas:** Pandas, NumPy, Matplotlib, Seaborn, Plotly, Excel Avançado

### **☁️ Cloud & Infraestrutura**
![Azure](https://img.shields.io/badge/Azure_ML-Intermediário-0078D4?logo=microsoftazure&logoColor=white)
![AWS](https://img.shields.io/badge/AWS-Básico-232F3E?logo=amazonaws&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-Básico-2496ED?logo=docker&logoColor=white)

### **🔄 Versionamento & Boas Práticas**
![Git](https://img.shields.io/badge/Git/GitHub-Expert-F05032?logo=git&logoColor=white)
![CI/CD](https://img.shields.io/badge/CI/CD-Conceitos-5ED3F3)

---

## 📚 Certificações Relevantes

| Certificação | Emissor | Foco | Status |
|--------------|---------|------|--------|
| **Google Advanced Data Analytics** | Google | Análise Preditiva & ML | ✅ Completa |
| **IBM Data Science Professional** | IBM | Ciência de Dados Completa | ✅ Completa |
| **Microsoft Azure Data Scientist** | Microsoft | ML na Nuvem | ✅ Completa |
| **Machine Learning with Python** | IBM | Algoritmos ML | ✅ Completa |
| **Business Intelligence** | Google | BI & Dashboarding | ✅ Completa |

*(+25 certificações em Data Science, Machine Learning e Analytics)*

---

## 🎓 Formação Acadêmica

- **MBA em Data Science e Analytics** | USP/ESALQ *(2021-2023)*  
- **MBA em Gestão de Projetos** | Universidade Anhanguera *(2009-2011)*  
- **Bacharelado em Ciência da Computação** | UNASP *(2004-2008)*  
- **Licenciatura em Matemática** | UNICV *(2024-2026)*

---

## 💼 Experiência Profissional Relevante

### **Bureau Works** *(2011 - Presente)*
**Technical & Creative Services Manager** | *14 anos em operações e estratégia*

- **Análise Preditiva:** Modelos para otimização de alocação de recursos
- **Business Intelligence:** Dashboards executivos com Power BI
- **Automação Analítica:** Pipeline de dados para decisão estratégica
- **Gestão Baseada em Dados:** KPIs, métricas e análise de performance

*(Posições anteriores incluíam Revenue Strategy, Quality Management e Project Management, todas com forte componente analítico)*

---

## 📁 Estrutura do Portfólio

```text
ivan-ajala-ds-portfolio/
├── projects/                    # Todos os projetos completos
│   ├── 01-churn-prediction/    # Sistema completo de ML
│   ├── 02-esg-analysis/        # Análise de dados públicos
│   └── 03-recommendation-system/ # Sistema de recomendação
├── docs/                       # Documentação detalhada
├── notebooks/                  # Jupyter notebooks de análise
├── src/                        # Código modularizado
├── datasets/                   # Dados utilizados (quando permitido)
├── assets/                     # Imagens e recursos visuais
├── LICENSE                     # MIT License
└── README.md                   # Este arquivo
```

**🎯 Filosofia dos Projetos:** Cada projeto simula um desafio real do mercado, incluindo:
- Definição do problema de negócio
- Coleta e preparação de dados
- Modelagem e validação
- Interpretabilidade e explicação
- Dashboard/API para consumo
- Documentação profissional

---

## 🔗 Conecte-se Comigo

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Ivan_Alala-0A66C2?logo=linkedin&logoColor=white)](https://linkedin.com/in/ivan-alala-ba759223)
[![GitHub](https://img.shields.io/badge/GitHub-IvanAjala-181717?logo=github&logoColor=white)](https://github.com/IvanAjala)
[![Email](https://img.shields.io/badge/Email-ivan_alala@hotmail.com-D14836?logo=gmail&logoColor=white)](mailto:ivan_alala@hotmail.com)
[![WhatsApp](https://img.shields.io/badge/WhatsApp-11_99196_9451-25D366?logo=whatsapp&logoColor=white)](https://wa.me/5511991969451)

**📍 Localização:** Osasco, São Paulo - Brasil  
**🎯 Status Profissional:** Aberto a oportunidades como **Cientista de Dados | Analista de Dados Sênior**  
**💼 Modalidade:** Presencial, Híbrido ou Remoto

---

## 📄 Licença

Este portfólio está licenciado sob a licença MIT - veja o arquivo [LICENSE](LICENSE) para detalhes.

---

## 🌟 Por Que Contratar um Profissional em Transição?

**"Profissionais que fazem transição de carreira trazem uma perspectiva única:  
eles entendem os dados E entendem o negócio. Essa combinação é rara e valiosa."**

Minha jornada de **14 anos em gestão** me proporcionou:
- Visão holística dos processos empresariais
- Habilidade de comunicar insights para todos os níveis
- Entendimento prático de métricas que realmente importam
- Experiência em implementação de soluções em escala

**Estou pronto para entregar valor desde o primeiro dia.**

---

*"Dados são apenas números até que alguém os transforme em histórias,  
e histórias são apenas palavras até que alguém as transforme em ação."*

**📞 Vamos conversar sobre como posso agregar valor à sua equipe de dados?**

---

**Última atualização:** Janeiro de 2024  
**Repositório ativo:** Novos projetos sendo adicionados mensalmente